...
class SNNDictionary3D(dt.AbstractDictionary): # shallow_neural_dict
    ...
    def _get_domain(self, param_b_domain):
        ...
        # 3D dictionary needs two polar coordinates t,p and the bias b     
    ...
    def _select_initial_elements(self, pde_energy):
        ...
        p = all_theta[1][sub[:,0:1],sub[:,1:2],sub[:,2:3]]
        b = all_theta[2][sub[:,0:1],sub[:,1:2],sub[:,2:3]]
        intial_guess = torch.cat([t,p,b], dim=1).to(self.device)
    ...
    def _get_optimizer(self, theta, optimizer_type):
        ...
        # theta contains w1, w2, w3 (weights) and b (bias), but w1, w2, w3 
        # are given by the two parameters in the polar coordinate
        t = Parameter(theta[0])
        p = Parameter(theta[1])
        b = Parameter(theta[2])
        params_input = [t,p,b]
